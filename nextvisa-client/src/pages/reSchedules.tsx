...
import { faCalendarAlt, faSearch, faFilter, faSpinner, faExclamationTriangle, faCheckCircle, faClock, faTimesCircle, faQuestionCircle } from '@fortawesome/free-solid-svg-icons';
...
const ReSchedules: React.FC = () => {
    ...
    // Fetch re-schedules
    const { data: reSchedules = [], isLoading, isError, error, refetch } = useReSchedules();
    ...
    const filteredReSchedules = useMemo(() => {
        ...
        return reSchedules.filter((item) => {
            ...
            const matchesApplicant = applicantFilter === 'ALL' || item.applicant === parseInt(applicantFilter);
    ...
    const getApplicantName = (applicantId: number) => {
        ...
        return applicant ? `${applicant.name} ${applicant.last_name}` : `ID: ${applicantId}`;
    };

    return (
        <div className="applicants-container"> {/* Reusing applicants container style */}
            <div className="applicants-header">
                <div>
                    <h1>Re-Schedules</h1>
                    <p>Monitor and manage appointment re-scheduling attempts</p>
                </div>
                <button className="add-btn" onClick={() => refetch()}>
